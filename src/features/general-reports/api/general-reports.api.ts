...
export const generalReportsApi = {
  getSummary: async (timeframe: string = '1w', startDate?: string, endDate?: string) => {
    const { data } = await api.get<DashboardSummary>('/general-reports/summary', { params: { timeframe, startDate, endDate } });
    return data;
  },

  getSpecialtyDistribution: async (timeframe: string = '1w', startDate?: string, endDate?: string) => {
    const { data } = await api.get<SpecialtyDistribution[]>('/general-reports/specialty-distribution', { params: { timeframe, startDate, endDate } });
    return data;
  },

  getDemographics: async (timeframe: string = '1w', startDate?: string, endDate?: string) => {
    const { data } = await api.get<PatientDemographics>('/general-reports/demographics', { params: { timeframe, startDate, endDate } });
    return data;
  },

  getTrends: async (timeframe: string = '1w', startDate?: string, endDate?: string) => {
    const { data } = await api.get<ConsultationTrend[]>('/general-reports/trends', { params: { timeframe, startDate, endDate } });
    return data;
  },

  getTopDiagnoses: async (timeframe: string = '1w', startDate?: string, endDate?: string) => {
    const { data } = await api.get<TopDiagnosis[]>('/general-reports/top-diagnoses', { params: { timeframe, startDate, endDate } });
    return data;
  },

  getGerenciasDistribution: async (timeframe: string = '1w', startDate?: string, endDate?: string) => {
    const { data } = await api.get<GerenciaDistribution[]>('/general-reports/gerencias-distribution', { params: { timeframe, startDate, endDate } });
    return data;
  },

  getAppointmentsStats: async (timeframe: string = '1w', startDate?: string, endDate?: string) => {
