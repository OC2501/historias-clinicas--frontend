...
export const getScheduleColumns = (
    ...
    const cols: Column<Schedule>[] = [
        ...
        },
        {
            header: 'Día',
            accessorKey: (schedule) => {
                const dayLabel = DAYS_OF_WEEK.find(d => d.value === schedule.diaSemana)?.label || 'Desconocido';
                if (schedule.fecha) {
                    const [year, month, dayVal] = schedule.fecha.split('-').map(Number);
                    const formattedDate = `${String(dayVal).padStart(2, '0')}/${String(month).padStart(2, '0')}/${year}`;
                    return (
                        <div className="flex flex-col">
                            <span className="font-medium text-slate-800 dark:text-slate-100">{dayLabel}</span>
                ...
                return (
                    <div className="flex flex-col">
                        <span className="font-medium text-slate-800 dark:text-slate-100">{dayLabel}</span>
                        <span className="text-[9px] text-sky-600 dark:text-sky-400 font-bold uppercase tracking-wider">Recurrente</span>
                    </div>
                );
            },
        },
